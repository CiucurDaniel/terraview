...
func AddImageLabel(graph *gographviz.Graph) {
	for _, node := range graph.Nodes.Nodes {
		...
		// If the label is empty or doesn't contain an underscore, skip the node
		if len(parts) < 2 {
			continue
		}

		// Construct the image label path
		// TODO: svg is not supported (on my computer by running dot cli)
		// Maybe a native go rendering library will support it
		imageLabel := filepath.Join("internal", "icons", "azurerm", parts[0]+".png")
